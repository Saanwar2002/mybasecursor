...
export async function DELETE(request: NextRequest) {
  ...
  const userId = searchParams.get('userId'); // The user requesting the deletion (must be the blocker)

  if (!blockId || !userId) {
    return NextResponse.json({ message: 'blockId and userId query parameters are required.' }, { status: 400 });
  }
