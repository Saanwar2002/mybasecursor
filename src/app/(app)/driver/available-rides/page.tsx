...
import { doc, setDoc, serverTimestamp, Timestamp, GeoPoint, collection, query, where, onSnapshot, Unsubscribe } from 'firebase/firestore';
...
export default function AvailableRidesPage() {
  ...
  const { user: driverUser, loading: authLoading } = useAuth();
  ...
  const [availableRideOffers, setAvailableRideOffers] = useState<RideOffer[]>([]);
  const [isFetchingOffers, setIsFetchingOffers] = useState(true);

  // Firestore listener for new ride offers
  useEffect(() => {
    if (!driverUser || !db) return;

    setIsFetchingOffers(true);

    const operatorCode = driverUser.operatorCode || PLATFORM_OPERATOR_CODE;
    
    const ridesRef = collection(db, 'rides');
    const q = query(
      ridesRef,
      where('status', '==', 'searching'),
      where('vehicleType', '==', driverUser.vehicleCategory || 'car'),
      where('originatingOperatorId', 'in', [operatorCode, PLATFORM_OPERATOR_CODE])
    );

    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      const offers: RideOffer[] = [];
      querySnapshot.forEach((doc) => {
        offers.push({ id: doc.id, ...doc.data() } as RideOffer);
      });
      setAvailableRideOffers(offers);
      setIsFetchingOffers(false);
    }, (error) => {
      console.error("Error fetching ride offers: ", error);
      toast({ title: "Error", description: "Could not fetch ride offers.", variant: "destructive" });
      setIsFetchingOffers(false);
    });
      ...
      watchIdRef.current = navigator.geolocation.watchPosition(
        ...
        (error) => {
          ...
          if (error.code === error.PERMISSION_DENIED || error.code === error.POSITION_UNAVAILABLE) {
            ...
            message = "Getting location timed out. Using default location.";
            setDriverLocation(huddersfieldCenterGoogle);
          }
  ...
  const handleAcceptOffer = async (rideId: string) => {
    ...
    try {
      ...
      const newActiveRideFromServer: ActiveRide = {
        ...
        driverVehicleDetails: serverBooking.driverVehicleDetails || `${serverBooking.vehicleType || 'Vehicle'} - Reg N/A`,
