          ...
          <AlertDialog open={isSosDialogOpen} onOpenChange={setIsSosDialogOpen}>
            ...
            <AlertDialogContent className="sm:max-w-md">
              ...
              <div className="grid grid-cols-2 gap-2 my-3">
                  {hazardTypes.filter(h => ['Emergency', 'Car Broken Down', 'Customer Aggressive', 'Call Me Back'].includes(h.label)).map(hazard => (
                      <Button key={hazard.id} onClick={() => handleQuickSOSAlert(hazard.label)} className={cn("font-bold", hazard.className)} size="sm"><span>{hazard.label}</span></Button>
                  ))}
              </div>
              <AlertDialogFooter>
