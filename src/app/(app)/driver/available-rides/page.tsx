...
import { MapPin, User, Clock, Check, X, Navigation, Route } from "lucide-react";
...
const mockRideRequests: RideRequest[] = [
  ...
  { id: 'r3', passengerName: 'Carol White', passengerAvatar: 'https://placehold.co/40x40.png?text=CW', pickupLocation: 'Green Park, London', dropoffLocation: 'Downtown Office, London', estimatedTime: '12 min', fareEstimate: 12.75, status: 'active', pickupCoords: [51.507, -0.142], dropoffCoords: [51.515, -0.087], distanceMiles: 3.1 },
];
        ...
        <Card className="bg-primary/10 border-primary/30 shadow-md">
          ...
          <CardContent className="space-y-2">
            ...
            {activeRide.distanceMiles && (
                <p className="flex items-center gap-1"><Route className="w-4 h-4 text-muted-foreground" /> <strong>Distance:</strong> {activeRide.distanceMiles.toFixed(2)} miles</p>
            )}
            <div className="mt-4">
        ...
        {pendingRides.map((req) => (
          <Card key={req.id} className="shadow-md hover:shadow-lg transition-shadow">
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              {req.distanceMiles && (
                <p className="flex items-center gap-1"><Route className="w-4 h-4 text-muted-foreground" /> <strong>Distance:</strong> {req.distanceMiles.toFixed(2)} miles</p>
              )}
            </CardContent>
