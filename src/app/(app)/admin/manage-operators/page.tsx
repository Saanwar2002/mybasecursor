...
export default function ManagePlatformOperatorsPage() {
  ...
  const handleOperatorStatusUpdate = async (operatorId: string, newStatus: OperatorUser['status'], reason?: string) => {
    if (!isPlatformAdminUser) {
      toast({ title: "Unauthorized", description: "Only the designated platform admin can update operator statuses.", variant: "destructive"});
      return;
    }
                    ...
                    <TableRow key={operator.id}>
                      ...
                      <TableCell className="text-center space-x-1">
                        {actionLoading[operator.id] ? (
                            ...
                            <>
                                {isPlatformAdminUser && operator.status === 'Pending Approval' && (
                                ...
                                {isPlatformAdminUser && (operator.status === 'Inactive' || operator.status === 'Suspended') && (
