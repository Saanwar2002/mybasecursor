...
export default function MyRidesPage() {
  ...
  const fetchDialogAddressSuggestions = useCallback((inputValue: string, setSuggestionsFunc: (s: google.maps.places.AutocompletePrediction[]) => void, setIsFetchingFunc: (isFetching: boolean) => void) => {
    if (!autocompleteServiceRef.current || inputValue.length < 2) { setSuggestionsFunc([]); setIsFetchingFunc(false); return; }
    ...
    autocompleteServiceRef.current.getPlacePredictions({ input: inputValue, sessionToken: autocompleteSessionTokenRef.current, componentRestrictions: { country: 'gb' } }, (predictions, status) => {
      ...
      if (status === google.maps.places.PlacesServiceStatus.OK && predictions) setSuggestionsFunc(predictions); else setSuggestionsFunc([]);
    });
  }, []);

  const handleDialogAddressInputChangeFactory = useCallback((formFieldNameOrStopIndex: 'pickupLocation' | 'dropoffLocation' | number) => (inputValue: string, formOnChange: (value: string) => void) => {
    formOnChange(inputValue); 
    if (typeof formFieldNameOrStopIndex === 'number') { 
      setDialogStopAutocompleteData(prev => prev.map((item, idx) => idx === formFieldNameOrStopIndex ? { ...item, inputValue, coords: null, suggestions: inputValue.length >= 2 ? item.suggestions : [], showSuggestions: inputValue.length >=2, isFetchingSuggestions: inputValue.length >=2 } : item));
      ...
      if(inputValue.length >=2) { setIsFetchingDialogPickupSuggestions(true); setDialogPickupSuggestions([]); } else { setIsFetchingDialogPickupSuggestions(false); setDialogPickupSuggestions([]); }
    } else { 
      setDialogDropoffInputValue(inputValue); setDialogDropoffCoords(null); setShowDialogDropoffSuggestions(inputValue.length >=2);
      if(inputValue.length >=2) { setIsFetchingDialogDropoffSuggestions(true); setDialogDropoffSuggestions([]); } else { setIsFetchingDialogDropoffSuggestions(false); setDialogDropoffSuggestions([]); }
    }
    if (debounceTimeoutRef.current) clearTimeout(debounceTimeoutRef.current);
    if (inputValue.length < 2) return;
    debounceTimeoutRef.current = setTimeout(() => {
      if (typeof formFieldNameOrStopIndex === 'number') fetchDialogAddressSuggestions(inputValue, (sugg) => setDialogStopAutocompleteData(prev => prev.map((item, idx) => idx === formFieldNameOrStopIndex ? { ...item, suggestions: sugg } : item)), (fetch) => setDialogStopAutocompleteData(prev => prev.map((item, idx) => idx === formFieldNameOrStopIndex ? { ...item, isFetchingSuggestions: fetch } : item)));
      else if (formFieldNameOrStopIndex === 'pickupLocation') fetchDialogAddressSuggestions(inputValue, setDialogPickupSuggestions, setIsFetchingDialogPickupSuggestions);
      else fetchDialogAddressSuggestions(inputValue, setDialogDropoffSuggestions, setIsFetchingDialogDropoffSuggestions);
    }, 300);
  }, [fetchDialogAddressSuggestions]);

  const handleDialogSuggestionClickFactory = useCallback((formFieldNameOrStopIndex: 'pickupLocation' | 'dropoffLocation' | number) => (suggestion: google.maps.places.AutocompletePrediction, formOnChange: (value: string) => void) => {
    const addressText = suggestion?.description; if (!addressText) return;
    ...
    if (placesServiceRef.current && suggestion.place_id) {
      placesServiceRef.current.getDetails({ placeId: suggestion.place_id, fields: ['geometry.location'], sessionToken: autocompleteSessionTokenRef.current }, (place, status) => {
        ...
        if (status === google.maps.places.PlacesServiceStatus.OK && place?.geometry?.location) setCoordsFunc({ lat: place.geometry.location.lat(), lng: place.geometry.location.lng() });
  ...
  const handleDialogFocusFactory = (formFieldNameOrStopIndex: 'pickupLocation' | 'dropoffLocation' | number) => () => {
    ...
    if (typeof formFieldNameOrStopIndex === 'number') { 
        ...
        if (currentInputValue.length >= 2 && currentSuggestions.length > 0) setDialogStopAutocompleteData(prev => prev.map((item, idx) => idx === formFieldNameOrStopIndex ? { ...item, showSuggestions: true } : item));
        else if (currentInputValue.length >= 2 && autocompleteServiceRef.current) fetchDialogAddressSuggestions(currentInputValue, (sugg) => setDialogStopAutocompleteData(prev => prev.map((item, idx) => idx === formFieldNameOrStopIndex ? { ...item, suggestions: sugg, showSuggestions: true } : item)), (fetch) => setDialogStopAutocompleteData(prev => prev.map((item, idx) => idx === formFieldNameOrStopIndex ? { ...item, isFetchingSuggestions: fetch } : item)));
        ...
        else if (currentInputValue.length >= 2 && autocompleteServiceRef.current) { fetchDialogAddressSuggestions(currentInputValue, setDialogPickupSuggestions, setIsFetchingDialogPickupSuggestions); setShowDialogPickupSuggestions(true); }
        ...
        else if (currentInputValue.length >= 2 && autocompleteServiceRef.current) { fetchDialogAddressSuggestions(currentInputValue, setDialogDropoffSuggestions, setIsFetchingDialogDropoffSuggestions); setShowDialogDropoffSuggestions(true); }
  ...
  const handleDialogBlurFactory = (formFieldNameOrStopIndex: 'pickupLocation' | 'dropoffLocation' | number) => () => {
    setTimeout(() => {
      if (typeof formFieldNameOrStopIndex === 'number') setDialogStopAutocompleteData(prev => prev.map((item, idx) => idx === formFieldNameOrStopIndex ? { ...item, showSuggestions: false } : item));
      else if (formFieldNameOrStopIndex === 'pickupLocation') setShowDialogPickupSuggestions(false);
      else setShowDialogDropoffSuggestions(false);
    }, 150); 
  };
  const handleAddEditStop = () => { appendEditStop({ location: "" }); setDialogStopAutocompleteData(prev => [ ...prev, { fieldId: `edit-stop-${prev.length}-${Date.now()}`, inputValue: "", suggestions: [], showSuggestions: false, isFetchingSuggestions: false, isFetchingDetails: false, coords: null }]); };
    ...
    if (user?.id) {
      const fetchRides = async () => {
        ...
        try {
          ...
          setRides(data);
        } catch (err) { const displayMessage = err instanceof Error ? err.message : "An unknown error occurred."; setError(displayMessage); toast({ title: "Error Fetching Rides", description: displayMessage, variant: "destructive", duration: 7000 });
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      ...
      if (!response.ok) { const errorData = await response.json().catch(() => ({ message: "Cancellation failed."})); throw new Error(errorData.message); }
  ...
  const handleOpenEditDetailsDialog = (ride: Ride) => {
    ...
    editDetailsForm.reset({
      pickupLocation: ride.pickupLocation.address, dropoffLocation: ride.dropoffLocation.address,
    ...
    setDialogPickupInputValue(ride.pickupLocation.address); setDialogPickupCoords({ lat: ride.pickupLocation.latitude, lng: ride.pickupLocation.longitude });
    setDialogDropoffInputValue(ride.dropoffLocation.address); setDialogDropoffCoords({ lat: ride.dropoffLocation.latitude, lng: ride.dropoffLocation.longitude });
    setDialogStopAutocompleteData(ride.stops?.map((s, i) => ({ fieldId: `edit-stop-initial-${i}`, inputValue: s.address, coords: { lat: s.latitude, lng: s.longitude }, suggestions: [], showSuggestions: false, isFetchingSuggestions: false, isFetchingDetails: false })) || []);
  ...
  async function onSubmitEditDetails(values: EditDetailsFormValues) {
    ...
    if (!dialogPickupCoords || !dialogDropoffCoords) { toast({ title: "Missing Location Coordinates", description: "Please ensure valid pickup and dropoff locations are selected from suggestions.", variant: "destructive" }); return; }
    const updatedStops: LocationPoint[] = [];
    for (let i = 0; i < (values.stops?.length || 0); i++) {
        const stopInput = values.stops?.[i]?.location; const stopData = dialogStopAutocompleteData[i];
        if (stopInput && stopInput.trim() !== "" && !stopData?.coords) { toast({ title: `Invalid Stop ${i + 1}`, description: `Please select stop ${i + 1} from suggestions or remove it.`, variant: "destructive" }); return; }
        if (stopData?.coords && stopInput && stopInput.trim() !== "") updatedStops.push({ address: stopInput, latitude: stopData.coords.lat, longitude: stopData.coords.lng });
    }
    setIsUpdatingDetails(true); let newScheduledPickupAt: string | null = null;
    if (values.desiredPickupDate && values.desiredPickupTime) {
      const [hours, minutes] = values.desiredPickupTime.split(':').map(Number);
      const combinedDateTime = new Date(values.desiredPickupDate); combinedDateTime.setHours(hours, minutes, 0, 0);
      newScheduledPickupAt = combinedDateTime.toISOString();
    }
    const payload = { bookingId: rideToEditDetails.id, passengerId: user.id, pickupLocation: { address: values.pickupLocation, latitude: dialogPickupCoords.lat, longitude: dialogPickupCoords.lng }, dropoffLocation: { address: values.dropoffLocation, latitude: dialogDropoffCoords.lat, longitude: dialogDropoffCoords.lng }, stops: updatedStops, scheduledPickupAt: newScheduledPickupAt };
    try {
      ...
      if (!response.ok) { const errorData = await response.json().catch(() => ({ message: "Failed to update details."})); throw new Error(errorData.message); }
      const updatedRideData = await response.json();
      setRides(prevRides => prevRides.map(r => r.id === rideToEditDetails.id ? { ...r, pickupLocation: updatedRideData.pickupLocation, dropoffLocation: updatedRideData.dropoffLocation, stops: updatedRideData.stops, scheduledPickupAt: updatedRideData.scheduledPickupAt } : r ));
      toast({ title: "Booking Details Updated", description: "Your ride details have been changed." });
      setIsEditDetailsDialogOpen(false);
    } catch (error) { toast({ title: "Update Failed", description: error instanceof Error ? error.message : "Unknown error.", variant: "destructive" });
  ...
  const handleAcknowledgeArrival = async (rideId: string) => {
    ...
    try {
        ...
        toast({ title: "Acknowledgement Failed", description: err instanceof Error ? err.message : "Unknown error.", variant: "destructive"});
    } finally {
  ...
  const renderDialogSuggestions = (suggestions: google.maps.places.AutocompletePrediction[], isFetchingSuggestions: boolean, isFetchingDetails: boolean, inputValue: string, onSuggestionClick: (suggestion: google.maps.places.AutocompletePrediction) => void, fieldKey: string ) => (
    <div className="absolute z-50 w-full mt-1 bg-card border rounded-md shadow-lg max-h-60 overflow-y-auto">
      ...
      {!isFetchingSuggestions && !isFetchingDetails && suggestions.map((s) => ( <div key={`${fieldKey}-${s.place_id}`} className="p-2 text-sm hover:bg-muted cursor-pointer" onMouseDown={() => onSuggestionClick(s)}>{s.description}</div> ))}
    </div>
  );
  ...
  if (isLoading) return ( <div className="space-y-6"><Card className="shadow-lg"><CardHeader><CardTitle className="text-3xl font-headline">My Rides</CardTitle><CardDescription>Loading your rides...</CardDescription></CardHeader></Card><div className="flex justify-center items-center py-10"><Loader2 className="h-12 w-12 animate-spin text-primary" /></div></div> );
  if (error && rides.length === 0) return ( <div className="space-y-6"><Card className="shadow-lg"><CardHeader><CardTitle className="text-3xl font-headline">My Rides</CardTitle><CardDescription>View past rides, rate experiences, manage bookings.</CardDescription></CardHeader></Card><Card className="border-destructive bg-destructive/10"><CardContent className="pt-6 text-center text-destructive"><AlertTriangle className="w-12 h-12 mx-auto mb-2" /><p className="font-semibold">Could not load rides.</p><p className="text-sm">{error}</p><Button variant="outline" onClick={() => window.location.reload()} className="mt-4">Try Again</Button></CardContent></Card></div> );

  return (
    <div className="space-y-6">
      <Card className="shadow-lg"><CardHeader><CardTitle className="text-3xl font-headline">My Rides</CardTitle><CardDescription>View past rides, rate experiences, manage bookings. ({rides.length} found)</CardDescription></CardHeader></Card>
          ...
          <Card key={ride.id} className="shadow-md hover:shadow-lg transition-shadow">
            <CardHeader>
              <div className="flex justify-between items-start">
                ...
                <Badge variant={ ride.status === 'completed' ? 'default' : ride.status === 'cancelled' ? 'destructive' : ride.status === 'in_progress' || ride.status === 'arrived_at_pickup' ? 'outline' : 'secondary' }
                  className={ cn( (ride.status === 'in_progress' || ride.status === 'arrived_at_pickup') && 'border-blue-500 text-blue-500', ride.status === 'pending_assignment' && 'bg-yellow-400/80 text-yellow-900', ride.status === 'driver_assigned' && 'bg-sky-400/80 text-sky-900', ride.status === 'completed' && 'bg-green-500/80 text-green-950', ride.status === 'cancelled' && 'bg-red-500/80 text-red-950' )}>
            ...
            <CardContent className="space-y-3">
              ...
              <Separator />
              {ride.status === 'arrived_at_pickup' && !ride.passengerAcknowledgedArrivalTimestamp && (
                <Alert variant="default" className="bg-blue-50 border-blue-400">
                  <BellRing className="h-5 w-5 text-blue-600" />
                  <ShadAlertTitle className="text-blue-700 font-semibold">Driver Has Arrived!</ShadAlertTitle>
                  <ShadAlertDescription className="text-blue-600">
                    Your driver is at the pickup location.
                    <Button 
                        size="sm" 
                        className="mt-2 w-full bg-blue-600 hover:bg-blue-700 text-white" 
                        onClick={() => handleAcknowledgeArrival(ride.id)}
                        disabled={actionLoading[ride.id]}
                    >
                        {actionLoading[ride.id] ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <CheckCheck className="mr-2 h-4 w-4"/>}
                        OK, I'm Coming!
                    </Button>
              ...
              {ride.status === 'arrived_at_pickup' && ride.passengerAcknowledgedArrivalTimestamp && (
                 <Alert variant="default" className="bg-green-50 border-green-400">
                    ...
                    <ShadAlertTitle className="text-green-700 font-semibold">You've Acknowledged Driver Arrival</ShadAlertTitle>
                    <ShadAlertDescription className="text-green-600">Your driver knows you are on your way.</ShadAlertDescription>
                 </Alert>
              )}
      ...
      {rideToEditDetails && ( <Dialog open={isEditDetailsDialogOpen} onOpenChange={setIsEditDetailsDialogOpen}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>Edit Booking Details</DialogTitle><DialogDescription>Modify pickup, dropoff, stops, or time. Fare may change (not recalculated here).</DialogDescription></DialogHeader><Form {...editDetailsForm}><form onSubmit={editDetailsForm.handleSubmit(onSubmitEditDetails)} className="space-y-4 max-h-[70vh] overflow-y-auto p-1 pr-3">
        ...
        <FormField control={editDetailsForm.control} name="dropoffLocation" render={({ field }) => ( <FormItem><FormLabel className="flex items-center gap-1"><MapPin className="w-4 h-4 text-muted-foreground" /> Dropoff</FormLabel><div className="relative"><FormControl><Input placeholder="Dropoff address" {...field} value={dialogDropoffInputValue} onChange={(e) => handleDialogAddressInputChangeFactory('dropoffLocation')(e.target.value, field.onChange)} onFocus={handleDialogFocusFactory('dropoffLocation')} onBlur={handleDialogBlurFactory('dropoffLocation')} autoComplete="off"/></FormControl>{showDialogDropoffSuggestions && renderDialogSuggestions(dialogDropoffSuggestions, isFetchingDialogDropoffSuggestions, isFetchingDialogDropoffDetails, dialogDropoffInputValue, (sugg) => handleDialogSuggestionClickFactory('dropoffLocation')(sugg, field.onChange), "dialog-dropoff")}</div><FormMessage /></FormItem>)} />
        <FormField control={editDetailsForm.control} name="desiredPickupDate" render={({ field }) => ( <FormItem className="flex flex-col"><FormLabel>New Pickup Date (Optional)</FormLabel><Popover><PopoverTrigger asChild><FormControl><Button variant={"outline"} className={cn("w-full pl-3 text-left font-normal", !field.value && "text-muted-foreground")}>{field.value ? format(field.value, "PPP") : <span>Pick a date</span>}<CalendarIconLucide className="ml-auto h-4 w-4 opacity-50" /></Button></FormControl></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={field.value} onSelect={(date) => { field.onChange(date); if (!date) editDetailsForm.setValue("desiredPickupTime", "");}} disabled={(date) => date < new Date(new Date().setHours(0,0,0,0)) } initialFocus/></PopoverContent></Popover><FormMessage /></FormItem>)} />
        <FormField control={editDetailsForm.control} name="desiredPickupTime" render={({ field }) => ( <FormItem><FormLabel>New Pickup Time (Optional)</FormLabel><FormControl><Input type="time" {...field} disabled={!editDetailsForm.watch("desiredPickupDate")}/></FormControl><FormMessage /></FormItem>)} />
        <DialogFooter className="pt-4 sticky bottom-0 bg-background pb-2"><DialogClose asChild><Button type="button" variant="outline" disabled={isUpdatingDetails || anyDialogFetchingDetails}>Cancel</Button></DialogClose><Button type="submit" disabled={isUpdatingDetails || anyDialogFetchingDetails} className="bg-primary hover:bg-primary/90 text-primary-foreground">{isUpdatingDetails && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save Changes</Button></DialogFooter>
